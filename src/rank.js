...
const YourComponent = () => {
  ...
  const handleConfirmClick = () => {
    alert('Your token SEED-' + tokenName +  ' is created!');
    togglePopup(); 
  };
  ...
  return (
    <div className="ranking-table-container">
      ...
      {isPopupOpen && (<div id="createTokenPopup" class="popup-overlay">
        <div class="popup-content">
          <h2>Create Token</h2>
          <p>Others can search and discover your token via the token symbol. Once the token is created, its symbol can't be changed.</p>
          <input type="text" id="tokenSymbol" placeholder="Please enter letters (A-Z)" value={tokenName} onChange={(e) => setTokenName(e.target.value)}/>
          <button id="confirmToken" onClick={handleConfirmClick}>Confirm</button>
          <button id="closePopup" class="close-popup-button" onClick={togglePopup}>✕</button>
        </div>
      </div>
      )}

      <button id="createButton" class="create-button" onClick={togglePopup}>Click Here to Create your Own Customized Brand Token!</button>
    </div>
  );
};
